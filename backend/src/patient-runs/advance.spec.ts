...
function startNode() {
  return {
    ...
    data: { kind: 'start' as const },
  };
}
function endNode(id = 'e') {
  return {
    id,
    position: { x: 30, y: START_Y },
...
describe('resolveAdvance', () => {
  it('start → single outgoing edge', () => {
    ...
    expect(resolveAdvance({ graph: g, currentNodeId: 's' })).toEqual({
      nextNodeId: 'a',
    });
  });
  ...
  it('send_* mode=simple with matching successCondition → success handle', () => {
    ...
    expect(
      resolveAdvance({ graph: g, currentNodeId: 'a', outcome: 'opened' }),
    ).toEqual({
      nextNodeId: 'e_ok',
      outcome: 'opened',
    });
  ...
  it('send_* mode=multi with matching status → corresponding handle', () => {
      ...
      outputs: [
        {
          id: 'engaged',
          label: 'Engagé',
          condition: { statuses: ['opened', 'clicked'] },
        },
        {
          id: 'rejected',
          label: 'Rejeté',
          condition: { statuses: ['bounced', 'rejected'] },
        },
      ],
    });
    const g: Graph = {
    ...
    expect(
      resolveAdvance({ graph: g, currentNodeId: 'a', outcome: 'clicked' }),
    ).toEqual({
      nextNodeId: 'e_eng',
      outcome: 'clicked',
    });
