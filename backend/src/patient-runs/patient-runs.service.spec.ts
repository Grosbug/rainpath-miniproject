...
describe('PatientRunsService', () => {
  ...
  async function seedWorkflowAndPatient() {
    ...
    const patient = await prisma.patientProfile.create({
      data: { firstName: 'Alice', lastName: 'Durand', gender: 'female' },
    });
    return { wf, patient };
  }
  ...
  it('create() seeds currentNodeId to start and history with one entry', async () => {
    ...
    const run = await service.create(wf.id, {
  ...
  it('get() returns the full run with parsed graph and patient', async () => {
    const { wf, patient } = await seedWorkflowAndPatient();
    const created = await service.create(wf.id, {
      patientId: patient.id,
      title: 'Détail',
    });
  ...
  it('advance() on an end node throws 400', async () => {
    ...
    await expect(service.advance(created.id, {})).rejects.toMatchObject({
      status: 400,
    });
  });
  ...
  it('parallel branches: enter each child at the same day then merge', async () => {
    ...
    const wf = await prisma.workflow.create({
      data: { name: 'Parallel', graph: JSON.stringify(parallelGraph) },
    });
    const patient = await prisma.patientProfile.create({
      data: { firstName: 'A', lastName: 'B', gender: 'female' },
    });
    const run = await service.create(wf.id, {
      patientId: patient.id,
      title: 'Parallèle',
    ...
    state = await service.advance(state.id, {
      nodeId: 'a',
      outcome: 'delivered',
    });
