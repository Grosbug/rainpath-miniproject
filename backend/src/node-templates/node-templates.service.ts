...
export class NodeTemplatesService {
   ...
   */
  private validateChannelRules(
    body: ReturnType<NodeTemplatesService['parseBody']>,
  ) {
    ...
    if (body.kind === 'send_email') channelKey = 'email';
      ...
      channelKey = body.params.tracked ? 'postal_tracked' : 'postal_untracked';
    ...
    if (output.mode === 'simple') {
      for (const s of output.successCondition.statuses) {
        if (!allowed.has(s))
          errors.push({
            code: 'status_not_in_channel',
            message: `status ${s} not in ${channelKey}`,
          });
      }
      ...
      for (const out of output.outputs) {
        for (const s of out.condition.statuses) {
          ...
          if (seen.has(s))
            errors.push({
              code: 'status_overlap_in_multi',
              message: `status ${s} appears in more than one output`,
            });
          seen.add(s);
        }
      }
