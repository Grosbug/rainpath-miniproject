...
describe('Workflows (e2e)', () => {
  ...
  it('POST /api/workflows creates a default workflow', async () => {
    const res = await request(app.getHttpServer())
      ...
      .expect(201);
    expect(
      res.body.graph.nodes.find((n: any) => n.data.kind === 'start').position,
  ...
  it('POST /api/workflows accepts an import with a cycle but surfaces the validationError', async () => {
    ...
    const res = await request(app.getHttpServer())
      ...
      .expect(201);
    expect(res.body.validationErrors.some((e: any) => e.code === 'cycle')).toBe(
      true,
    );
  });

  it('GET /api/workflows returns the paginated envelope and omits graph in items', async () => {
    ...
    expect(res.body.items[0].graph).toBeUndefined();
    expect(res.body.items[0].name).toBe('A');
    expect(typeof res.body.items[0].isValid).toBe('boolean');
  });

  it('GET /api/workflows?limit=1&offset=1 paginates correctly', async () => {
    for (const name of ['A', 'B', 'C']) {
      await request(app.getHttpServer()).post('/api/workflows').send({ name });
    }
    const res = await request(app.getHttpServer())
  ...
  it('GET /api/workflows?limit=99999 is rejected (max 200)', async () => {
    await request(app.getHttpServer())
      .get('/api/workflows?limit=99999')
      .expect(422);
  });

  it('GET /api/workflows/:id 404s on unknown id', async () => {
    await request(app.getHttpServer())
      .get('/api/workflows/no-such-id')
      .expect(404);
  });
  ...
  it('DELETE /api/workflows/:id soft-deletes (returns 204; list no longer shows it)', async () => {
    ...
    const list = await request(app.getHttpServer())
      ...
      .expect(200);
    expect(
      list.body.items.find((w: any) => w.id === created.body.id),
    ).toBeUndefined();
  });
});
