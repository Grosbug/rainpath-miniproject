...
/**
 * Nodes that may be entered next: every incoming predecessor has finished
 * routing (`isExitedForGating`), the node itself is not visited, and its
 * **runtime** entry day equals the minimum across all ready nodes (strict
 * temporal gate — finish all of J+N before any J+M with M > N).
 *
 * For convergent join targets, this defers opening until every incoming
 * branch has recorded its outcome — so the simulator can attribute the
 * join's entry day to the slowest branch that fed it.
 */
export function computeActiveFrontiers(graph: Graph, history: RunHistoryEntry[]): string[] {
  ...
  for (const n of graph.nodes) {
    ...
    if (!incoming.every(e => isExitedForGating(graph, history, e.source))) continue
    ready.push(n.id)
  }
