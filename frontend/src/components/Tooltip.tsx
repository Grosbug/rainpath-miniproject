...
export function Tooltip() {
  ...
  useEffect(() => {
    function resolve(el: EventTarget | null): { text: string } | null {
    ...
    function onOver(e: MouseEvent) {
      ...
      if (!match) return
      setTip({ text: match.text, x: e.clientX, y: e.clientY })
    }
  ...
  return (
    <div
      role='tooltip'
      className='pointer-events-none fixed z-[1100] whitespace-nowrap rounded-full bg-fg px-3 py-1 text-xs font-medium text-bg shadow-elev-2'
